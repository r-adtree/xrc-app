...
// Upload URL (requires token)
const getHFUploadUrl = (repo) => `https://huggingface.co/api/datasets/${repo}/upload/${HF_FILE_PATH}`;
  ...
  const handleCSV = e=>{
    ...
    reader.onload=async ev=>{
      try{
        ...
        // Upload to Hugging Face (replace mode)
        const blob = new Blob([text], {type:"text/csv"});
        const formData = new FormData();
        formData.append("files", blob, HF_FILE_PATH);
        const res = await fetch(getHFUploadUrl(cfg.repo), {
          method: "POST",
        ...
        if(!res.ok) {
          ...
          throw new Error(`HF upload gagal (${res.status}): ${err.slice(0,120)}`);
        }
        setHotels(parsed);
