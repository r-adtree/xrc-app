...
// Upload URL — HF Hub API (commit/create_file)
const getHFUploadUrl = (repo) => `https://huggingface.co/api/datasets/${repo}/raw/main/${HF_FILE_PATH}`;
  ...
  const handleCSV = e=>{
    ...
    reader.onload=async ev=>{
      try{
        ...
        // Upload to Hugging Face via Hub API — PUT raw file content
        ...
        if(!res.ok) {
          ...
          let msg = errText.slice(0,200).replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
