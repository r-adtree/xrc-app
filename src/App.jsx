...
// ─── GIST CONFIG ─────────────────────────────────────────────────────────────
// Admin: create a GitHub Gist at gist.github.com, paste your CSV, set to PUBLIC.
// Copy the RAW url (click Raw button) and paste below.
// Format: https://gist.githubusercontent.com/USERNAME/GIST_ID/raw/hotels.csv
const GIST_RAW_URL_KEY = "adtree_gist_url";
const getGistUrl = () => localStorage.getItem(GIST_RAW_URL_KEY) || "";
const setGistUrl = url => localStorage.setItem(GIST_RAW_URL_KEY, url);

// ─── Default hotel data (fallback if no Gist configured) ─────────────────────
...
export default function App() {
  ...
  const [gistUrl,   setGistUrlSt] = useState(getGistUrl);
  ...
  const [gistInput, setGistInput] = useState(getGistUrl);
  const fileRef = useRef();

  // ── Fetch from Gist or use default ──────────────────────────────────────────
  useEffect(()=>{
    const url = getGistUrl();
    if(url) {
      setLoading(true);
      // Add cache-bust to always get latest
      fetch(url+"?cb="+Date.now())
        .then(r=>{ if(!r.ok) throw new Error("Gagal fetch Gist"); return r.text(); })
        .then(text=>{
  ...
  const handleCSV = e=>{
    ...
    reader.onload=ev=>{
      try{
        ...
        if(!valid.length) throw new Error("Tidak ada data valid.");
        const parsed=valid.map(csvRowToHotel).map(enrichHotel).sort((a,b)=>b.score-a.score);
        setHotels(parsed);
        setCsvStatus("success"); setCsvMsg(`✓ ${parsed.length} brand diimport. Data lokal diperbarui.`);
      }catch(err){ setCsvStatus("error"); setCsvMsg("✗ Error: "+err.message); }
    };
    reader.readAsText(file,"UTF-8"); e.target.value="";
  };

  const saveGistUrl = ()=>{
    if(!gistInput.includes("gist.githubusercontent.com")&&gistInput.trim()){
      setCsvMsg("✗ URL harus dari gist.githubusercontent.com/..."); setCsvStatus("error"); return;
    }
    setGistUrl(gistInput); setGistUrlSt(gistInput);
    if(gistInput.trim()){
      setCsvStatus("loading"); setCsvMsg("Mengambil data dari Gist...");
      fetch(gistInput+"?cb="+Date.now())
        .then(r=>{if(!r.ok)throw new Error("Gagal fetch");return r.text();})
        .then(text=>{
          const rows=parseCSV(text);
          const valid=rows.filter(r=>(r.poi_name||r.name||"").trim());
          if(!valid.length)throw new Error("Tidak ada data valid di Gist.");
          const parsed=valid.map(csvRowToHotel).map(enrichHotel).sort((a,b)=>b.score-a.score);
          setHotels(parsed);
          setCsvStatus("success"); setCsvMsg(`✓ ${parsed.length} brand diambil dari Gist. Semua user akan melihat data ini saat buka app!`);
        })
        .catch(err=>{ setCsvStatus("error"); setCsvMsg("✗ "+err.message); });
    } else {
      setHotels(DEFAULT_HOTELS_RAW.map(enrichHotel).sort((a,b)=>b.score-a.score));
      setCsvStatus("success"); setCsvMsg("✓ Gist URL dihapus. Kembali ke data default.");
    }
  };
  ...
  if(view==="adminPanel"&&isAdmin) return (
    <div style={{...S.root,background:"#0C1220"}}>
      ...
      <div style={{display:"flex",borderBottom:`1px solid ${BRAND.cardBorder}`}}>
        {[["list","📋 Brand"],["upload","📤 CSV"],["gist","🔗 Gist"],["add","➕ Tambah"]].map(([id,lbl])=>(
      ...
      {adminTab==="gist"&&(
        <div style={{padding:"20px 20px 90px"}}>
          <div style={{fontSize:14,fontWeight:800,color:BRAND.text,marginBottom:6}}>🔗 GitHub Gist — Live Data untuk Semua User</div>
          <div style={{fontSize:12,color:BRAND.textMuted,lineHeight:1.7,marginBottom:20}}>
            Dengan Gist, setiap kali kamu update CSV di GitHub, semua user yang buka app akan otomatis dapat data terbaru — tanpa perlu deploy ulang ke Vercel.
          </div>

          {/* Steps */}
          {[
            ["1","Buat akun GitHub","github.com (gratis)"],
            ["2","Buat Gist baru","gist.github.com → New Gist → nama file: hotels.csv → paste CSV kamu → Create public gist"],
            ["3","Copy RAW URL","Klik tombol Raw → copy URL dari address bar browser"],
            ["4","Paste URL di bawah","Format: https://gist.githubusercontent.com/USERNAME/ID/raw/hotels.csv"],
          ].map(([n,title,desc])=>(
          ...
          <label style={S.formLabel}>Gist RAW URL</label>
          <input value={gistInput} onChange={e=>setGistInput(e.target.value)} placeholder="https://gist.githubusercontent.com/..." style={{...S.formInput,marginBottom:10,fontSize:11}} onKeyDown={e=>e.key==="Enter"&&saveGistUrl()}/>
          ...
          <button onClick={saveGistUrl} style={{...S.cta,marginBottom:10}}>💾 Simpan & Tarik Data dari Gist</button>
          {gistInput&&<button onClick={()=>{setGistInput("");setGistUrl("");localStorage.removeItem(GIST_RAW_URL_KEY);setCsvStatus(null);setCsvMsg("");}} style={{display:"block",width:"100%",padding:"12px",borderRadius:12,border:`1px solid ${BRAND.cardBorder}`,background:"transparent",color:BRAND.textMuted,fontSize:13,fontWeight:600,cursor:"pointer"}}>✕ Hapus Gist URL</button>}

          <div style={{background:"#F5C84208",border:`1px solid ${BRAND.yellowBorder}`,borderRadius:12,padding:"12px 14px",marginTop:16}}>
            <div style={{fontSize:11,fontWeight:700,color:BRAND.yellow,marginBottom:4}}>💡 Cara Update Data ke Semua User</div>
            <div style={{fontSize:11,color:BRAND.textMuted,lineHeight:1.7}}>
              1. Edit CSV di GitHub Gist (klik Edit → update → Save changes)<br/>
              2. Semua user yang buka/refresh app akan otomatis dapat data baru<br/>
              3. Tidak perlu deploy ulang Vercel sama sekali
            </div>
          </div>
          ...
          <div style={{background:"#1A2640",borderRadius:12,padding:"12px 14px",marginTop:12,border:`1px solid ${BRAND.cardBorder}`}}>
            ...
            <div style={{fontSize:11,color:gistUrl?BRAND.green:BRAND.textMuted}}>{gistUrl?"✓ Gist aktif: "+gistUrl.slice(0,50)+"...":"✗ Belum dikonfigurasi — app pakai data default"}</div>
          </div>
        </div>
      )}
      ...
      {adminTab==="upload"&&(
        <div style={{padding:"20px 20px 90px"}}>
          ...
          <div style={{fontSize:12,color:BRAND.textMuted,marginBottom:16,lineHeight:1.6}}>Upload untuk preview/test di perangkat ini. Untuk update semua user, gunakan tab <span style={{color:BRAND.yellow,fontWeight:700}}>🔗 Gist</span>.</div>
      ...
      <nav style={S.nav}>
        {[["📋","Brand","list"],["📤","Upload","upload"],["🔗","Gist","gist"],["➕","Tambah","add"],["🚪","Keluar","logout"]].map(([ic,lb,id])=>(
